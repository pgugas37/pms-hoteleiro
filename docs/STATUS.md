...
## Módulo 22 — Tarifas sazonais/promocionais ✅

Concluído e validado. Nova tabela `seasonal_rates` (migration `module22_seasonal_rates`) — diária diferente do preço base, válida num intervalo de datas, por tipo de quarto (ex.: alta temporada, Réveillon, feriados).

- Colunas: `room_type_id` (referencia `room_types`, `on delete cascade`), `label`, `start_date`, `end_date`, `daily_rate`. RLS igual ao padrão de `room_types`: leitura pra todo autenticado, escrita só admin.
- **Trava de sobreposição no banco**: constraint de exclusão (`exclude using gist`, mesma extensão `btree_gist` já usada na trava anti-overbooking do Módulo 07) impede cadastrar dois períodos que se sobrepõem pro mesmo tipo de quarto — testado direto via SQL antes de liberar (rollback, sem deixar dado de teste).
- **Frontend:** nova seção "Tarifas sazonais" na tela **Quartos** (abaixo de Tipos de quarto e Quartos), com listagem e cadastro/edição via modal (só admin edita).
- **Integração com Reservas:** ao escolher o quarto e/ou a data de check-in (em qualquer ordem) na tela de Reservas — tanto na reserva individual quanto na reserva em grupo —, a diária sugerida passa a checar primeiro se existe uma tarifa sazonal cobrindo aquela data pro tipo do quarto; se não houver, cai no preço base do tipo, como já era. Continua totalmente editável manualmente. Reservas já existentes não são afetadas — o cálculo só entra ao criar/editar.
...
## Módulo 24 — Busca global ✅

Concluído e validado. Módulo só de frontend, sem migration nova — o objetivo é economizar cliques no dia a dia: em vez de entrar em Hóspedes, Quartos ou Reservas pra procurar algo, dá pra buscar de qualquer tela.

- **`src/components/GlobalSearchDialog.tsx`** (novo): botão "Buscar" no cabeçalho (`AppLayout.tsx`) e atalho de teclado Ctrl/Cmd+K, disponíveis em qualquer tela autenticada. Busca com debounce de 300ms, a partir de 2 caracteres, em paralelo: hóspedes (nome ou documento, via `ilike`/`or` no Supabase), quartos (número) e reservas (por hóspede ou por quarto — resolvido buscando primeiro os ids de hóspedes/quartos que batem com o termo, depois as reservas vinculadas a esses ids). O termo de busca é higienizado (remove `%`, `_`, `,`, `(`, `)`) antes de virar filtro do PostgREST.
- Resultados agrupados por tipo (Hóspedes / Quartos / Reservas); clicar num resultado navega pra `/hospedes`, `/quartos` ou `/reservas` já com o termo aplicado via parâmetro `?q=` na URL, que cada tela lê ao carregar (`useSearchParams`) pra pré-preencher o próprio campo de busca.
- A tela **Quartos** ganhou uma caixa de busca própria (número, tipo ou andar) que não existia antes. A busca de **Reservas** passou a considerar também o número do quarto, além do nome do hóspede.
...
## Módulo 26 — Motivo de cancelamento / no-show ✅

Concluído e validado. Hoje o cancelamento de reserva só mudava o status pra "cancelada" sem registrar o porquê — esse módulo dá rastreabilidade sobre quantos cancelamentos são no-show (hóspede simplesmente não aparece) x avisados com antecedência, sem automatizar cobrança de multa (decisão de negócio caso a caso do Gustavo, fora de escopo por enquanto).

- Migration `module26_cancellation_reason`: colunas novas `cancellation_reason` (`hospede_avisou` / `no_show` / `outro`, com `check`) e `cancellation_notes` em `reservations`, com uma segunda constraint garantindo que só reserva com status `cancelada` pode ter motivo preenchido. A função RPC `cancel_reservation` precisou ser recriada (a assinatura mudou de 1 pra 3 parâmetros — `p_reservation_id`, `p_reason`, `p_notes`) com os mesmos grants explícitos das outras RPCs (revoga de `anon`, libera só pra `authenticated`, checagem de papel dentro da função). Testado direto no banco antes de liberar: cancelamento com motivo grava certo, e as duas constraints de validação bloqueiam corretamente motivo em reserva não cancelada e motivo inválido.
- **Frontend (`ReservationsPage.tsx`):** o botão "Cancelar" abre um dialog pedindo o motivo (select) e uma observação opcional, em vez do `window.confirm` simples de antes. Reservas confirmadas com check-in já vencido (e sem check-in feito) ganham um badge "Atrasada" na lista, e o dialog de cancelamento já vem com "No-show" pré-selecionado nesses casos (mas pode trocar). O motivo do cancelamento passou a aparecer na lista (embaixo do status) e no dialog de edição da reserva.
